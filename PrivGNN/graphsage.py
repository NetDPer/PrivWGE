...
class GraphSageBase(tf.keras.Model):
    """
    GraphSage base model outputting embeddings of given nodes
    """
...
class MeanAggregator(tf.keras.layers.Layer):
    ...
    def call(self, dstsrc_features, dstsrc2src, dstsrc2dst, dif_mat):
        ...
        # -------------------------------------------------------------------
        matrix_2norm = tf.norm(concatenated_features, ord=2)
        # matrix_2norm = tf.norm(concatenated_features, ord='fro')
        # Normalization
        epsilon = 1e-8
        normalized_features = concatenated_features / (matrix_2norm + epsilon)
        sigma = 5
        noisy_features = normalized_features + tf.compat.v1.random_normal(tf.shape(normalized_features), stddev=sigma)
        # --------------------------------------------------------------------
        x = tf.matmul(noisy_features, self.w)
