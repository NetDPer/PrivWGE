...
def build_batch_from_edges(trainGraph, edges, nodes, neigh_dict, sample_sizes, neg_size):
    """
    ...
    minibatch_plain = build_batch_from_nodes ( trainGraph, batch_all
                                             , neigh_dict
                                             , sample_sizes
                                             )

    MiniBatchFields = [ "src_nodes", "dstsrc2srcs", "dstsrc2dsts", "dif_mats"
                      , "dst2batchA", "dst2batchB", "dst2batchN", "batch_all" ]
...
def build_batch_from_nodes(trainGraph, nodes, neigh_dict, sample_sizes):
    """
    ...
    for sample_size in reversed(sample_sizes):
        ds, d2s, d2d, dm = _compute_diffusion_matrix ( trainGraph, dst_nodes[-1]
                                                     , neigh_dict
                                                     , sample_size
                                                     , max_node_id
                                                     )
...
def _compute_diffusion_matrix(trainGraph, dst_nodes, neigh_dict, sample_size, max_node_id):
    G = trainGraph
    # def sample(ns):
    #     # ns: list of (neighbor, weight)
    #     if len(ns) == 0:
    #         return []
    #     elif len(ns) <= sample_size:
    #         return ns
    #     else:
    #         idx = np.random.choice(len(ns), sample_size, replace=False)
    ...
    def vectorize(ns, center):
        v = np.zeros(max_node_id + 1, dtype=np.float32)
        for n in ns:
            # 查找权重
            if G.has_edge(center, n):
                w = G.edges[center, n].get('weight', 1.0)
            else:
                w = 0.0
            v[n] = w
        return v

    # sample neighbors with weights
    # adj_mat_full = np.stack([vectorize(sample(neigh_dict[n])) for n in dst_nodes])
    # sample neighbors with weights
    adj_mat_full = np.stack([vectorize(sample(neigh_dict[n]), n) for n in dst_nodes])
    ...
    dif_mat = adj_mat / (adj_mat_sum + 1e-8)  # 防止除零
